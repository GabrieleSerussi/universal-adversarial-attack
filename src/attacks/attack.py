...
class Attack:
    ...
    def compute_train_eval_clean_output(self, data_loader, y_list, eval_data_loader, eval_y_list, device=None):
        #eval_data_loader is called as None (at least initially)
        #eval_y_list is called as None (at least initially)
    ...
    def compute_clean_baseline(self, data_loader, y_list, eval_data_loader, eval_y_list, device=None):
        ...
        best_loss_avg = np.average([loss[-1].item() for loss in eval_clean_loss_list])
    ...
    def calc_sample_grad(self, pert, img1_I0, img2_I0, intrinsic_I0, img1_delta, img2_delta,
                         scale, y, clean_flow, target_pose, perspective1, perspective2, mask1, mask2, device=None, req_F=False):
        if not req_F:
            grad = self.calc_sample_grad_aux(pert, img1_I0, img2_I0, intrinsic_I0, img1_delta, img2_delta,
                         scale, y, clean_flow, target_pose, perspective1, perspective2, mask1, mask2, device, req_F=False)
            return grad
        else:
            grad, loss = self.calc_sample_grad_aux(pert, img1_I0, img2_I0, intrinsic_I0, img1_delta, img2_delta,
                                             scale, y, clean_flow, target_pose, perspective1, perspective2, mask1,
                                             mask2, device, req_F=True)
            return grad, loss
    ...
    def attack_eval(self, pert, data_shape, eval_data_loader, eval_y_list, device, isTrain=False, avg=False):
        with torch.no_grad():
            loss_list = []
            loss_sum_list = []
            ...
            for data_idx, data in enumerate(eval_data_loader):
                ...
                img1_adv, img2_adv, output_adv = self.perturb_model(pert_expand, img1_I0, img2_I0,
                                                                    ...
                                                                    device)
                if isTrain:
                    loss = self.criterion(output_adv, scale.to(device),
            ...
            if not isTrain:
                if avg:
                    del loss_sum_list
                    avg = np.average(np.array(loss_list)[:,-1])
                    torch.cuda.empty_cache()
                    return avg, loss_list
                else:
    ...
    def gradient_ascent_step(self, pert, data_shape, data_loader, y_list, clean_flow_list,
                             multiplier, a_abs, eps, d=None, requires_grad=False, req_f=False, device=None):

        pert_expand = pert.expand(data_shape[0], -1, -1, -1).to(device)
        grad_tot = torch.zeros_like(pert, requires_grad=False)

        if d is not None:
            d["since_last_check"] += 1

        if (d is not None and "g" not in d) or (d is None):
            for data_idx, data in enumerate(data_loader):
                ...
                if not req_f:
                    if d is None:
                        ...
                        grad = self.calc_sample_grad(d["x"].expand(data_shape[0], -1, -1, -1).to(device), img1_I0, img2_I0, intrinsic_I0,
                                                 img1_delta, img2_delta,
                                                 scale, y_list[data_idx], clean_flow_list[data_idx], patch_pose,
                                                 perspective1, perspective2,
                                                 mask1, mask2, device=device)
                else:
                    grad, f = self.calc_sample_grad(pert_expand, img1_I0, img2_I0, intrinsic_I0,
                                                 img1_delta, img2_delta,
                                                 scale, y_list[data_idx], clean_flow_list[data_idx], patch_pose,
                                                 perspective1, perspective2,
                                                 mask1, mask2, device=device, req_F=True)
        ...
        if d is not None:
            ...
            del grad_tot
            z = self.project(d["x"]+d["lr"]*grad, eps)
            x_next = self.project(d["x"]+
                                  0.75*(z-d["x"])+
                                  0.25*(d["x"]-d["x_prev"]), eps)
            g, f_next = self.gradient_ascent_step(x_next, data_shape, data_loader, y_list, clean_flow_list,
                             multiplier, a_abs, eps, requires_grad=False, req_f=True, device=device)
            ...
            d["x"] = x_next
            if d["iter"] in d["W"]:
                condition1 = (d["improved"]/d["since_last_check"]) < 0.75
                condition2 = d["lr_changed"] and (d["improved"] > 0)
                if condition1 or condition2:
                    d["lr"] = d["lr"]/2
                    d["lr_changed"] = True
                    d["x"] = d["x_max"]
                    d["since_last_check"] = 0
                    d["improved"] = 0
                    if "g" in d:
                        del d["g"]
                else:
        ...
        if not requires_grad and d is None:
            with torch.no_grad():
                grad = self.normalize_grad(grad_tot)
                if req_f:
                    del grad_tot
            ...
            with torch.no_grad():
                return self.normalize_grad(grad_tot)
