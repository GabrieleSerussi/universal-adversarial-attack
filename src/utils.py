...
def compute_data_args(args):
    ...
    args.test_dir = f'{abs_path}/data/{args.test_dir}'

    if args.processed_data_dir is None:
        args.processed_data_dir = f"{abs_path}/data/{args.test_dir_name}_processed"
    ...
    args.testDataset = \
        args.dataset_class(args.test_dir, processed_data_folder=args.processed_data_dir,
                                     ...
                                     folder_indices_list = [2,4,6,7,8,9],
                                     max_traj_datasets=args.max_traj_datasets, cancel_preprocessed_data=(not args.preprocessed_data))

    args.testDataloader = DataLoader(args.testDataset, batch_size=args.batch_size,
                                        shuffle=True, num_workers=args.worker_num)


    #changed by me! The following will be the VALIDATION dataset (the PREVIOUS one is the TRAINING dataset)
    args.validationDataset = \
        args.dataset_class(args.test_dir, processed_data_folder=args.processed_data_dir,
                                     ...
                                     centerx=args.centerx, centery=args.centery, max_traj_len=args.max_traj_len,
                                     max_dataset_traj_num=args.max_traj_num,
                                     folder_indices_list = [3,5],
                                     max_traj_datasets=args.max_traj_datasets)

    args.validationDataloader = DataLoader(args.validationDataset, batch_size=args.batch_size,
                                        shuffle=True, num_workers=args.worker_num)

    args.realTestDataset = \
        args.dataset_class(args.test_dir, processed_data_folder=args.processed_data_dir,
                           preprocessed_data=args.preprocessed_data,
                           transform=args.transform, data_size=(args.image_height, args.image_width),
                           focalx=args.focalx, focaly=args.focaly,
                           centerx=args.centerx, centery=args.centery, max_traj_len=args.max_traj_len,
                           max_dataset_traj_num=args.max_traj_num,
                           folder_indices_list=[0,1],
                           max_traj_datasets=args.max_traj_datasets)

    args.realTestDataloader = DataLoader(args.realTestDataset, batch_size=args.batch_size,
                                           shuffle=True, num_workers=args.worker_num)
