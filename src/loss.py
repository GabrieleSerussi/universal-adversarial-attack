...
def test_model(model, criterion, img1, img2, intrinsic, scale_gt, motions_target, target_pose,
    ...
    '''It outputs the estimated motions, the estimated optical flow and the value of the criterion'''
...
class VOCriterion:
    def __init__(self, t_crit='mean_partial_rms', rot_crit='none', flow_crit='none', target_t_crit='none',
                 t_factor=1.0, rot_factor=0.0, flow_factor=0.0, target_t_factor=0.0):
        ...
        epsilon = 1e-6
        self.calc_rot_crit = self.rot_calc_mean_partial_poses_t

        #by using a lambda function I can change the dimension over which the cosine similarity is computed!
        # add 1+epsilon and apply logarithm to improve the numerical stability
        self.calc_flow_crit = lambda x1, x2: torch.sum(
                                                torch.log(
                                                    1+epsilon-torch.nn.functional.cosine_similarity(x1,x2,dim=1)
                                                    ),dim=(1,2)
        )

        self.calc_target_t_product = False
    def load(self, criterion_state):
        self.t_factor = criterion_state["trans"]
        self.rot_factor = criterion_state["rot"]
        self.flow_factor = criterion_state["flow"]
    ...
    def apply(self, model_output, scale, motions_gt, target_pose, flow_clean=None):
        ...
        flow_crit = torch.zeros(flow.shape[0] + 1,
                                device=flow.device, dtype=flow.dtype)
        rot_crit[1:] = scale * self.calc_rot_crit(motions, motions_gt)
        
        flow_crit[1:] = scale * self.calc_flow_crit(flow, flow_clean) if flow_clean is not None else 0
    ...
    def rot_calc_mean_partial_poses_t(self, motions, motions_gt):
        rel_poses = self.rtvec_to_pose(motions)
        rel_poses_gt = self.rtvec_to_pose(motions_gt)
        t_errors_tot = torch.zeros(rel_poses.shape[0] + 1,
                                   device=rel_poses.device, dtype=rel_poses.dtype)
        for traj_s_idx in range(rel_poses.shape[0]):
            ...
            cumul_poses = self.cumulative_poses(partial_traj)
            cumul_poses_gt = self.cumulative_poses(partial_traj_gt)
            t_error = self.rotation_error(cumul_poses, cumul_poses_gt)
            t_errors_tot[:max_traj_size] += t_error
        t_errors_traj_num = list(range(rel_poses.shape[0] + 1, 0, -1))
        t_errors_traj_num = torch.tensor(t_errors_traj_num, device=rel_poses.device, dtype=rel_poses.dtype)
        t_errors_mean = t_errors_tot / t_errors_traj_num
        return t_errors_mean[1:]
    ...
    def rotation_error(self, cumul_poses, cumul_poses_gt):
        cumul_delta_t = cumul_poses[:, 0:3, 0:2] - cumul_poses_gt[:, 0:3, 0:2]
        t_error = torch.norm(cumul_delta_t, p="fro", dim=[1,2])
        return t_error
