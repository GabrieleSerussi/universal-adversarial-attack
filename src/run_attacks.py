...
def test_model(model, criterions, img1, img2, intrinsic, scale_gt, motions_target, target_pose=None,
    ...
    if window_size is None:
        ...
        #changed by me! print flow_device
        # print('flow device:')
        # print(flow_device)
        #changed by me! print criterions
        # print('criterions:')
        # for crit in criterions:
        #     print(type(crit))
        #     print([f'{i}:{getattr(crit,i)}' for i in dir(crit)])
...
def test_clean_multi_inputs(args, type='train'):
    dataset_idx_list = []
    dataset_name_list = []
    traj_name_list = []
    traj_indices = []
    ...
    if type is None or type == 'train':
        dataloader = args.testDataloader
        print("len(args.testDataloader)")
    elif type =='valid':
        dataloader = args.validationDataloader
        print("len(args.validationDataloader)")
    elif type == 'test':
        dataloader = args.realTestDataloader
        print("len(args.realTestDataloader)")
    else:
    ...
    print("traj_mask_l0_ratio_std over all (" + str(len(traj_name_list)) + ") traperujectories:")
...
def report_adv_deviation(dataset_idx_list, dataset_name_list, traj_name_list, traj_indices,
                         ...
                         experiment_name="", type=None):
    traj_len = len(traj_clean_crit_list[0])
    if type is None or type == 'train':
        csv_path = os.path.join(output_dir, 'results_' + crit_str + '.csv')
    elif type == 'valid':
        csv_path = os.path.join(output_dir, 'val_' + 'results_' + crit_str + '.csv')
    elif type == 'test':
        csv_path = os.path.join(output_dir, 'test' + 'results_' + crit_str + '.csv')
    else:
        raise AttributeError
...
def run_attacks_train(args):
    ...
    motions_gt_list, traj_clean_criterions_list, traj_clean_motions = \
        test_clean_multi_inputs(args, type='train')

    #Validation
    val_dataset_idx_list, val_dataset_name_list, val_traj_name_list, val_traj_indices, \
    val_motions_gt_list, val_traj_clean_criterions_list, val_traj_clean_motions = \
        test_clean_multi_inputs(args, type='valid')

    # Test
    real_test_dataset_idx_list, real_test_dataset_name_list, real_test_traj_name_list, real_test_traj_indices, \
    real_test_motions_gt_list, real_test_traj_clean_criterions_list, real_test_traj_clean_motions = \
        test_clean_multi_inputs(args, type='test')
    ...
    # Validation
    val_traj_clean_rms_list, val_traj_clean_mean_partial_rms_list, \
    val_traj_clean_target_rms_list, val_traj_clean_target_mean_partial_rms_list = tuple(val_traj_clean_criterions_list)

    # Test
    real_test_traj_clean_rms_list, real_test_traj_clean_mean_partial_rms_list, \
    real_test_traj_clean_target_rms_list, real_test_traj_clean_target_mean_partial_rms_list = tuple(real_test_traj_clean_criterions_list)
    ...
    best_pert, clean_loss_list, all_loss_list, all_best_loss_list = \
        attack.perturb(args.testDataloader, motions_target_list, eps=args.eps, device=args.device,
                       eval_data_loader=args.validationDataloader, eval_y_list=val_motions_gt_list)
    ...
    # Validation
    val_traj_adv_criterions_list = \
        test_adv_trajectories(args.validationDataloader, args.model, val_motions_gt_list, attack, best_pert,
                              args.criterions, args.window_size,
                              args.save_imgs, args.save_flow, args.save_pose,
                              args.adv_img_dir, args.adv_pert_dir, args.flowdir, args.pose_dir,
                              device=args.device)

    val_traj_adv_rms_list, val_traj_adv_mean_partial_rms_list, \
    val_traj_adv_target_rms_list, val_traj_adv_target_mean_partial_rms_list = tuple(val_traj_adv_criterions_list)

    #Test

    real_test_traj_adv_criterions_list = \
        test_adv_trajectories(args.realTestDataloader, args.model, real_test_motions_gt_list, attack, best_pert,
                              args.criterions, args.window_size,
                              args.save_imgs, args.save_flow, args.save_pose,
                              args.adv_img_dir, args.adv_pert_dir, args.flowdir, args.pose_dir,
                              device=args.device)

    real_test_traj_adv_rms_list, real_test_traj_adv_mean_partial_rms_list, \
    real_test_traj_adv_target_rms_list, real_test_traj_adv_target_mean_partial_rms_list = tuple(real_test_traj_adv_criterions_list)
    ...
    report_adv_deviation(dataset_idx_list, dataset_name_list, traj_name_list, traj_indices,
                         ...
                         args.save_csv, args.output_dir, crit_str="mean_partial_rms", type="train")

    report_adv_deviation(dataset_idx_list, dataset_name_list, traj_name_list, traj_indices,
                         traj_clean_rms_list, traj_adv_rms_list,
                         args.save_csv, args.output_dir, crit_str="rms", type="train")

    report_adv_deviation(val_dataset_idx_list, val_dataset_name_list, val_traj_name_list, val_traj_indices,
                         val_traj_clean_mean_partial_rms_list, val_traj_adv_mean_partial_rms_list,
                         args.save_csv, args.output_dir, crit_str="mean_partial_rms", type="valid")

    report_adv_deviation(val_dataset_idx_list, val_dataset_name_list, val_traj_name_list, val_traj_indices,
                         val_traj_clean_rms_list, val_traj_adv_rms_list,
                         args.save_csv, args.output_dir, crit_str="rms", type="valid")

    report_adv_deviation(real_test_dataset_idx_list, real_test_dataset_name_list, real_test_traj_name_list, real_test_traj_indices,
                         real_test_traj_clean_mean_partial_rms_list, real_test_traj_adv_mean_partial_rms_list,
                         args.save_csv, args.output_dir, crit_str="mean_partial_rms", type="test")

    report_adv_deviation(real_test_dataset_idx_list, real_test_dataset_name_list, real_test_traj_name_list, real_test_traj_indices,
                         real_test_traj_clean_rms_list, real_test_traj_adv_rms_list,
                         args.save_csv, args.output_dir, crit_str="rms", type="test")
